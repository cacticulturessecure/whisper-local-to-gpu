...
class LiveAudioChunker:
    def __init__(self, device_info, chunk_seconds=15):
        ...
        self.complete_dir = Path("complete_recordings")
    ...
    def _process_audio(self):
        ...
        while self.is_recording or not self.audio_queue.empty():
            if not self.audio_queue.empty():
                ...
                self.complete_recording.extend(data.flatten())
    ...
    def _save_complete_recording(self):
        """Save the complete recording as a single WAV file"""
        if not self.complete_recording:
            return
            
        timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
        filename = self.complete_dir / f"complete_recording_{timestamp}.wav"
        
        # Convert to numpy array
        complete_audio = np.array(self.complete_recording)
        
        with wave.open(str(filename), 'wb') as wf:
            wf.setnchannels(self.channels)
            wf.setsampwidth(4)  # 32-bit float
            wf.setframerate(self.rate)
            wf.writeframes(complete_audio.astype(self.dtype).tobytes())
            
        # Add to stats
        self.complete_recording_path = filename
    ...
    def get_stats(self):
        ...
        # Add complete recording info if available
        if hasattr(self, 'complete_recording_path'):
            stats["complete_recording"] = str(self.complete_recording_path)
...
def display_stats(chunker):
    ...
    if "complete_recording" in stats:
        table.add_row(
            "Complete Recording",
            str(stats["complete_recording"])
        )
...
def main():
    ...
    try:
        ...
        with Live(display_stats(chunker), refresh_per_second=4) as live:
            ...
            console.print(f"[cyan]Saving chunks to: {chunker.output_dir}[/cyan]")
            console.print(f"[cyan]Complete recording will be saved to: {chunker.complete_dir}[/cyan]")
            input()
    ...
    if hasattr(chunker, 'complete_recording_path'):
        console.print(f"\n[green]Complete recording saved to: {chunker.complete_recording_path}[/green]")
    console.print(f"[green]Chunks saved in: {Path('live_chunks').absolute()}[/green]")
